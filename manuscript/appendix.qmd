...
The original dataset of the *Survey of COVID-19 related Behaviours and Attitudes* included different covariates (age, age-group, employment, remote work within the last month, etc). However, we are interested in the following covariates:
...
The data was cleaned in order to keep the covariates, determine their differences to the 2016 Census Data, and identify and remove outliers. All analyses were carried in RStudio (2022.12.0 Build 353) and R version 4.2.2.

```{r,load-data,warning=FALSE,echo=FALSE}

#the dataframe has missing answers as blank spaces, need to change the blank spaces all to NA
...
## Covariate Missigness

Next, we determined the number of missing observations in the covariates.
